...
class BranchServiceClientTest extends GeneratedTest
{
    ...
    public function getBranchTest()
    {
        ...
        $response = $gapicClient->getBranch($request);
    ...
    public function listBranchesTest()
    {
        ...
        $response = $gapicClient->listBranches($request);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
    ...
    public function listBranchesExceptionTest()
    {
        ...
        $request = (new ListBranchesRequest())
            ->setParent($formattedParent);
        try {
            $gapicClient->listBranches($request);
            // If the $gapicClient method call did not throw, fail the test
            $this->fail('Expected an ApiException, but no exception was thrown.');
        } catch (ApiException $ex) {
            $this->assertEquals($status->code, $ex->getCode());
    ...
    public function getBranchAsyncTest()
    {
        ...
        $request = (new GetBranchRequest())
            ->setName($formattedName);
        $response = $gapicClient->getBranchAsync($request)->wait();
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
