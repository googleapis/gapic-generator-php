...
namespace Testing\BasicAutoPopulation\Tests\Unit;
...
class BasicAutoPopulationClientTest extends GeneratedTest
{
    ...
    public function createFooTest()
    {
        ...
        $response = $gapicClient->createFoo($aField);
    ...
    public function getFooTest()
    {
        ...
        // Mock request
        $aField = 'aField-1289259108';
        $response = $gapicClient->getFoo($aField);
        $this->assertEquals($expectedResponse, $response);
        $actualRequests = $transport->popReceivedCalls();
        $this->assertSame(1, count($actualRequests));
        $actualFuncCall = $actualRequests[0]->getFuncCall();
