...
class GapicClientGenerator
{
    ...
    {
        // No point in adding sendAsync to a client that only has streaming RPCs that aren't supported
        // by startAsyncCall.
    ...
    private function rpcMethod(MethodDetails $method): PhpClassMember
    {
        ...
        return AST::method($method->methodName)
            ...
            ->withPhpDoc(PhpDoc::block(
                ...
                $isStreamedRequest
                    ? null
                    : PhpDoc::preFormattedText(Vector::new(['To invoke this method asynchronously {@see sendAsync}.'])),
                $isStreamedRequest
                    ? null
                    : Vector::zip(
                        $method->requiredFields->filter(fn ($f) => !$f->isOneOf || $f->isFirstFieldInOneof()),
